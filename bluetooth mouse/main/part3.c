...
void hid_demo_task(void *pvParameters) {
    ...
    while (1) {
        ...
        // Adjust base speed according to tilt
        if (x_accel > NORMAL && y_accel > NORMAL) {
            x_move = -SPEED;
            y_move = SPEED;
            if (x_accel > FAST && y_accel > FAST) {
                x_move *= MULT;  // Double the speed
                y_move *= MULT;  // Double the speed
            }
            esp_hidd_send_mouse_value(hid_conn_id, 0, x_move, y_move);
        } else if (x_accel < -NORMAL && y_accel > NORMAL) {
            x_move = SPEED;
            y_move = SPEED;
            if (x_accel < -FAST && y_accel > FAST) {
                x_move *= MULT;
            ...
            esp_hidd_send_mouse_value(hid_conn_id, 0, x_move, y_move);
        } else if (x_accel > NORMAL) {
            x_move = -SPEED;
            if (x_accel > FAST) {
                x_move *= MULT;
            }
            esp_hidd_send_mouse_value(hid_conn_id, 0, x_move, 0);
        } else if (x_accel < -NORMAL) {
            x_move = SPEED;
            if (x_accel < -FAST) {
                x_move *= MULT;
            ...
            esp_hidd_send_mouse_value(hid_conn_id, 0, x_move, 0);
        } else if (y_accel > NORMAL) {
            y_move = SPEED;
            if (y_accel > FAST) {
                y_move *= MULT;
            }
            esp_hidd_send_mouse_value(hid_conn_id, 0, 0, y_move);
        } else if (y_accel < -NORMAL) {
            y_move = -SPEED;
            if (y_accel < -FAST) {
                y_move *= MULT;
...
void app_main(void)
{
    ...
    i2c_master_init();
    icm42670_setup();  // Setup sensor after initializing I2C
