...
//         esp_http_client_set_url(client, "http://192.168.86.34:1234");
//         esp_http_client_set_method(client, HTTP_METHOD_POST);
//         esp_http_client_set_header(client, "Content-Type", "application/json");
//         esp_http_client_set_post_field(client, post_data, strlen(post_data));
...
void temp_task(char* weather_info) {
    uint8_t data[6];
    char *phone_address = "http://169.233.174.202:1234";
    ...
    esp_http_client_handle_t client = esp_http_client_init(&config);
        ...
        if (calculate_crc(data, 2) == data[2] && calculate_crc(data+3, 2) == data[5]) {
            ...
            char post_data[1000];
            snprintf(post_data, sizeof(post_data), "Local temperature is %.0fC (or %.0fF) with a %.0f%% humidity\n\n%s",
                     temperature, temperatureF, humidity, weather_info);

            esp_http_client_set_url(client, phone_address);
            esp_http_client_set_method(client, HTTP_METHOD_POST);
            esp_http_client_set_header(client, "Content-Type", "application/json");
            esp_http_client_set_post_field(client, post_data, strlen(post_data));
